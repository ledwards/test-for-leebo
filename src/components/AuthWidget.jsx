...
export default function AuthWidget({ showOnlyWhenLoggedIn = false }) {
  ...
  useEffect(() => {
    console.log('AuthWidget: Auth state - user:', user, 'loading:', loading, 'showOnlyWhenLoggedIn:', showOnlyWhenLoggedIn)
  }, [user, loading, showOnlyWhenLoggedIn])
  ...
  if (!user) {
    ...
    return (
      <div className="auth-widget">
        <button className="auth-widget-login-button" onClick={signIn}>
          <svg 
            width="20" 
            height="20" 
            viewBox="0 0 24 24" 
