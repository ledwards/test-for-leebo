// @ts-nocheck
/**
 * Draft Remove Route - Validation Logic Tests
 *
 * Tests the authorization and validation logic for removing
 * players from a draft lobby (host-only, waiting status only).
 */
...
function validateRemovePlayer(pod, session, playerId, player) {
  ...
  if (pod.status !== 'waiting') {
    return { error: 'Cannot remove players after draft has started', status: 400 }
  }
  if (!player) {
    return { error: 'Player not found in this draft', status: 404 }
  }
  if (player.user_id === session.id) {
    return { error: 'Cannot remove yourself. Delete the draft instead.', status: 400 }
  }
  return null // Valid
}

describe('Draft Remove - Validation Logic', () => {
  ...
  it('rejects when pod not found', () => {
    ...
    assert.strictEqual(result.error, 'Draft not found')
  })
  ...
  it('rejects when draft is active (not waiting)', () => {
    const activePod = { ...basePod, status: 'active' }
    const result = validateRemovePlayer(activePod, hostSession, otherPlayer.id, otherPlayer)
    assert.strictEqual(result.status, 400)
    assert.match(result.error, /after draft has started/)
  })

  it('rejects when draft is complete', () => {
    const completePod = { ...basePod, status: 'complete' }
    const result = validateRemovePlayer(completePod, hostSession, otherPlayer.id, otherPlayer)
    assert.strictEqual(result.status, 400)
  })
  ...
  it('works for removing bot players (bot has a user_id too)', () => {
...
describe('Draft Remove - playerId vs userId', () => {
  it('uses pod_players.id (row PK) not user_id for lookup', () => {
    ...
    // Simulating the correct lookup: WHERE id = playerId
    ...
    // Simulating the WRONG lookup: WHERE user_id = playerId
